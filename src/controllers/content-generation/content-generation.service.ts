...
export class ContentGenerationService {
  ...
  async generateReview(): Promise<{
    ...
    const reviewText = await this.generateContent(
      'Напиши позитивный отзыв о приложении для казино на русском языке. Пиши, как обычный пользователь, используй простой язык, короткие слова. Не добавляй свои комментарии, только отзыв. Два предложения. Важно, чтобы не было видно, что это AI писал. Начало предложения:',
      100,
      [
