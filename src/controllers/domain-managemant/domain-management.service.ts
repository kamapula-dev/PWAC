...
export class DomainManagementService {
    ...
    async addDomain(email: string, apiToken: string, domain: string): Promise<any> {
        // Step 1: Add the domain to Cloudflare
        const zoneId = await this.addZone(apiToken, domain);

        // Step 2: Deploy Worker for the domain
        await this.deployWorker(apiToken, zoneId, domain, 'cdn-worker/dist/index.js');

        // Step 3: Return instructions for DNS configuration
        return {
            message: 'Domain successfully added and worker deployed. Update NS records as follows:',
            nsRecords: [
    ...
    private async addZone(apiToken: string, domain: string): Promise<string> {
        try {
            ...
            if (response.data.success) {
                ...
                throw new Error(response.data.errors[0].message || 'Failed to add domain to Cloudflare');
            }
        } catch (error) {
            console.error('Error adding domain:', error.message);
            throw new HttpException('Failed to add domain to Cloudflare', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }

    private async deployWorker(apiToken: string, zoneId: string, domain: string, scriptPath: string): Promise<void> {
        try {
            ...
            throw new HttpException('Failed to deploy worker', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
